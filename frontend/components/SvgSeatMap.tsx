...
export function SvgSeatMap({ seats, reservedSeatIds, selectedSeatId, roomCode, onSelectSeat }: SvgSeatMapProps) {
  ...
  return (
    <div className="relative min-h-[460px] overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
      <div className="absolute left-4 top-4 z-10 flex items-center gap-2 rounded-xl border border-slate-200 bg-white/95 p-1 shadow-sm">
        <button type="button" onClick={() => zoom(0.15)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100" title={t.zoomIn}>
          <ZoomIn size={18} />
        </button>
        <button type="button" onClick={() => zoom(-0.15)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100" title={t.zoomOut}>
          <ZoomOut size={18} />
        </button>
        <button type="button" onClick={reset} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100" title={t.resetView}>
          <RotateCcw size={18} />
        </button>
        <span className="hidden items-center gap-1 border-l border-slate-200 px-3 text-xs font-semibold text-slate-500 sm:flex">
          <Move size={14} />
          {Math.round(scale * 100)}%
        </span>
      </div>

      <div className="absolute right-4 top-4 z-10 hidden rounded-xl border border-slate-200 bg-white/95 px-3 py-2 text-xs font-semibold text-slate-500 shadow-sm sm:block">
        {t.dragHint}
      </div>
      ...
      <div className="absolute bottom-4 left-4 z-10 flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white/95 px-3 py-2 text-xs font-semibold text-slate-600 shadow-sm">
        ...
        <span className="inline-flex items-center gap-1"><span className="h-3 w-3 rounded-full bg-slate-400" />{t.reserved}</span>
      </div>
    </div>
  );
}
...
function LabRoomBackground() {
  return (
    <g>
      ...
      <text x="779" y="254" textAnchor="middle" className="fill-slate-950 text-[16px] font-black">ENTRY</text>
    </g>
  );
}
...
function MeetingRoomBackground() {
  return (
    <g>
      <path fill="#DCEAF7" stroke="#366286" strokeWidth="5" d="M82.5 455.5V52.5h766v403z" />
      <rect x="235" y="130" width="505" height="150" rx="32" fill="#FCFCFC" stroke="#C0BECA" strokeWidth="2" />
      <rect x="265" y="160" width="445" height="90" rx="22" fill="#E2E8F0" stroke="#94A3B8" />
      <path fill="#D9D9D9" d="M83 258v-64h64v64z" />
      <path fill="#D9D9D9" d="m147 226 56-32v64l-56-32Z" />
      <text x="330" y="370" className="fill-slate-600 text-[22px] font-bold">CS MEETING ROOM</text>
    </g>
  );
}
